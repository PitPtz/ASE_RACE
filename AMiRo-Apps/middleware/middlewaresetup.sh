...
function printWelcomeText {
  printf "######################################################################\n"
  printf "#                                                                    #\n"
  printf "#                Welcome to the uRT submodule setup!                 #\n"
  printf "#                                                                    #\n"
  printf "######################################################################\n"
  printf "#                                                                    #\n"
...
function printHelp {
  ...
  printf "          -i, --init\n"
  printf "              Initialize the uRT submodule.\n"
  printf "          -d, --documentation <option>\n"
  printf "              Possible options are: 'g' and 'o' (can be combined).\n"
  printf "              - g: Generate HTML documentation of µRT.\n"
  printf "              - o: Open HTML documentation of µRT.\n"
  printf "          -w, --wipe\n"
  printf "              Wipe the entire uRT submodule.\n"
  printf "          --quit\n"
...
### initialize uRT submodule ###################################################
# Initialize the uRT submodule.
#
# usage:      initUrt
# arguments:  n/a
# return:     0
...
#
function initUrt {
  printInfo "initializing uRT submodule...\n"
  local userdir=$(pwd)
  local middlewaredir=$(dirname $(realpath ${BASH_SOURCE[0]}))
  local urtdir=${middlewaredir}/uRT

  # if the uRT folder is not empty
  if [ ! -z "$(ls -A $urtdir)" ]; then
    printWarning "$(realpath $urtdir) is not empty. Delete and reinitialize? [y/n]\n"
    local userinput=""
    ...
    case $userinput in
      Y|y)
        ...
        ;;
      N|n)
        printWarning "uRT initialization aborted by user\n"
        return 1
        ;;
  ...
  cd $middlewaredir
  git submodule update --init $urtdir 2>&1 | tee -a $LOG_FILE
...
#                 No error or warning occurred.
#             1
#                 Warning: µRT not initialized yet.
#             2
...
function documentation {
  ...
  # if the µRT folder is empty
  if [ -z "$(ls -A $urtdir)" ]; then
    printWarning "$urtdir is empty. Please initialize first.\n"
    return 1
  else
    local option="";
    # if no argument was specified, ask what to do
    if [ $# -eq 0 ]; then
      printInfo "µRT documentation setup\n"
      ...
      option=${userinput,,}
      if [ $option = $'\e' ]; then
        printInfo "µRT documentation setup aborted by user\n"
        return 2
      fi
    else
      option="$1"
    ...
    case "$option" in
      ...
      G|g)
        ...
        if [ -f ${urtdir}/docs/doxygen/Doxyfile ]; then
          printInfo "generating µRT documentation...\n"
          cd ${urtdir}/docs/doxygen
          doxygen Doxyfile 2>&1 | tee -a $LOG_FILE &> /dev/null
          cd $userdir
          printInfo "access µRT documentation via ${urtdir}docs/doxygen/output/html/index.html\n"
        else
          issues=$((issues + 1))
          printError "could not generate µRT documentation\n"
        fi
        ;;
      ...
      O|o)
        # check if required files exist
        if [ -f ${urtdir}/docs/doxygen/output/html/index.html ]; then
          printInfo "open µRT documentation\n"
          xdg-open ${urtdir}/docs/doxygen/output/html/index.html &> /dev/null &
        else
          issues=$((issues + 1))
          printError "could not open µRT documentation\n"
        fi
        ;;
...
### reset uRT submodule and wipe directory #####################################
# Resets the uRT Git submodule and wipes the directory.
#
# usage:      wipeUrt
# arguments:  n/a
# return:     0
#                 No error or warning occurred.
#             1
#                 Warning: uRT Git submodule already empty
#             2
...
function wipeUrt {
  ...
  local middlewaredir=$(dirname $(realpath ${BASH_SOURCE[0]}))
  local urtdir=${middlewaredir}/uRT
  printInfo "reset and wipe Git submodule $urtdir\n"
  ...
  # if the uRT folder is empty
  if [ -z "$(ls -A $urtdir)" ]; then
    printWarning "$urtdir is already empty\n"
    return 1
  else
    # get some information from Git
    cd $middlewaredir
    local git_basehash=($(git ls-tree -d HEAD $urtdir)); git_basehash=${git_basehash[2]}
    cd $urtdir
    ...
    if [ $issues -gt 0 ]; then
      ...
      case "$userinput" in
        Y|y)
          ;;
        N|n)
          printfWarning "wiping uRT Git submodule aborted by user\n"
          return 2
          ;;
        *) # sanity check (return error)
          printError "unexpected input: $userinput\n"; return -1;;
    ...
    cd $middlewaredir
    git submodule update --force --checkout $urtdir | tee -a $LOG_FILE
    cd $urtdir
    ...
    # deinitialize uRT submodule and delete any remaining files
    cd $middlewaredir
    git submodule deinit -f $urtdir 2>&1 | tee -a $LOG_FILE
    rm -rf $urtdir/*
    cd $userdir
...
# Initializes or wipes the uRT Git submodule, and provides an entry point to its
# setup.
#
...
function main {
  ...
  while ( true ); do
    ...
    printf "Please select one of the following actions:\n"
    printf "   [I]  - initialize uRT submodule\n"
    printf "   [D]  - generate or open HTML documentation\n"
    printf "   [W]  - wipe uRT submodule\n"
