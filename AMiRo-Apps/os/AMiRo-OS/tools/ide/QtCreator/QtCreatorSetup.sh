...
#                 If not set, the AMiRo-OS root directory is suggested.
# return:     n/a
#
function getProjectDir {
  ...
  # if no initial path was given, default to AMiRo-OS root directory
  if [ -z "$amiroosdir" ]; then
    amiroosdir=$(realpath $(dirname $(realpath ${BASH_SOURCE[0]}))/../../../)
  fi

  printLog "reading path for project files from user...\n"
  read -p "Path where to create/delete project folders & files: " -i $amiroosdir -e input
...
function createProject {
  ...
  if [ -z "$projectdir" ]; then
    getProjectDir projectdir $(realpath $(dirname $(realpath ${BASH_SOURCE[0]}))/../../../qtcreator)
    printf "\n"
  fi
