...
/**
 * @file    module.c
 * @brief   Structures and constant for the DiWheelDrive v1.1 module.
 */
...
ROMCONST SPIConfig moduleHalSpiAccelerometerConfig = {
  /* circular buffer mode             */ false,
  /* slave mode                       */ false,
  /* data callback function pointer   */ NULL,
  /* error callback function pointer  */ NULL,
  /* chip select line                 */ LINE_ACCEL_SS_N,
  /* CR1                              */ SPI_CR1_BR_0,
  /* CR2                              */ SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN,
};

ROMCONST SPIConfig moduleHalSpiGyroscopeConfig = {
  /* circular buffer mode             */ false,
  /* slave mode                       */ false,
  /* data callback function pointer   */ NULL,
  /* error callback function pointer  */ NULL,
  /* chip select line                 */ LINE_GYRO_SS_N,
  /* CR1                              */ SPI_CR1_BR_0,
  /* CR2                              */ SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN,
};
...
void moduleInitTests()
{
  ...
  aosShellAddCommand(&moduleTestHmc5883lShellCmd);
  aosShellAddCommand(&moduleTestIna219ShellCmd);
  aosShellAddCommand(&moduleTestL3g4200dShellCmd);
  aosShellAddCommand(&moduleTestLedShellCmd);
  aosShellAddCommand(&moduleTestLis331dlhShellCmd);
 ...
 * HMC5883L (compass)
 */
#include "test/HMC5883L/module_test_HMC5883L.h"
static int _moduleTestHmc5883lShellCmdCb(BaseSequentialStream* stream, int argc, const char* argv[])
{
  return moduleTestHmc5883lShellCb(stream, argc, argv, NULL);
}
AOS_SHELL_COMMAND(moduleTestHmc5883lShellCmd, "test:Compass", _moduleTestHmc5883lShellCmdCb);
 ...
 * L3G4200D (gyroscope)
 */
#include "test/L3G4200D/module_test_L3G4200D.h"
static int _moduleTestL3g4200dShellCmdCb(BaseSequentialStream* stream, int argc, const char* argv[])
{
  return moduleTestL3g4200dShellCb(stream, argc, argv, NULL);
}
AOS_SHELL_COMMAND(moduleTestL3g4200dShellCmd, "test:Gyroscope", _moduleTestL3g4200dShellCmdCb);
 ...
 */
#include "test/LIS331DLH/module_test_LIS331DLH.h"
static int _moduleTestLis331dlhShellCmdCb(BaseSequentialStream* stream, int argc, const char* argv[])
{
  return moduleTestLis331dlhShellCb(stream, argc, argv, NULL);
}
AOS_SHELL_COMMAND(moduleTestLis331dlhShellCmd, "test:Accelerometer", _moduleTestLis331dlhShellCmdCb);
...
static int _moduleTestAllShellCmdCb(BaseSequentialStream* stream, int argc, const char* argv[])
{
  ...
  /* HMC5883L (compass) */
  status |= moduleTestHmc5883lShellCb(stream, 0, (const char**)targv, &result_test);
  result_total = aosTestResultAdd(result_total, result_test);
  ...
  /* L3G4200D (gyroscope) */
  status |= moduleTestL3g4200dShellCb(stream, 0, (const char**)targv, &result_test);
  result_total = aosTestResultAdd(result_total, result_test);
  ...
  /* LIS331DLH (accelerometer) */
  status |= moduleTestLis331dlhShellCb(stream, 0, (const char**)targv, &result_test);
  result_total = aosTestResultAdd(result_total, result_test);
