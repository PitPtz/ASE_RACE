...
void moduleInitTests()
{
  ...
  aosShellAddCommand(&moduleTestSwitchShellCmd);
 ...
 * switch
 */
#include "test/switch/module_test_switch.h"
static int _moduleTestSwitchShellCmdCb(BaseSequentialStream* stream, int argc, const char* argv[])
{
  return moduleTestSwitchShellCb(stream, argc, argv, NULL);
}
AOS_SHELL_COMMAND(moduleTestSwitchShellCmd, "test:Switch", _moduleTestSwitchShellCmdCb);
...
static int _moduleTestAllShellCmdCb(BaseSequentialStream* stream, int argc, const char* argv[])
{
  ...
  /* battery switch */
  status |= moduleTestSwitchShellCb(stream, 0, (const char**)targv, &result_test);
  result_total = aosTestResultAdd(result_total, result_test);
